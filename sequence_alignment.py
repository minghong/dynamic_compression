...
def pretty_print_align(seq1, seq2, path_code,v):
    '''
    ...
    for p in path_code:
        if p == '0':
            ...
            middle = middle + ' '
            seq1 = seq1[1:]
    out_1=open("refer.txt","w")
    out_2=open("sequence.txt","w")
    
    out_1.write(align1 + '\n')
    out_2.write(align2 + '\t'+str(v)+"\n")
    return


def main(seq1,seq2,v):
    ...
    pretty_print_align(seq1, seq2, path_code,v)
    #print('   '+path_code)
...
if __name__ == '__main__':
    ...
    for line in file.readlines():
        result_a.append(line.rstrip("\n"))
    file.close()
    result_b={}
    file=open("1_unique.fq")
    for line in file.readlines():
        tmp=line.rstrip("\n").split("\t")[0]
        result_b[tmp[0]]=tmp[1]
    file.close()
    
    for item_a in result_a:
        for item_b,v in result_b.items():
            ratio = fuzz.ratio(item_a, item_b)
            if ratio > 80:
                main(item_a,item_b,v)